...
import { InterfaceTypingsModule } from '../interface-typings'
...
class BotInterfacesModule extends ReExportTypeModule {
  public constructor(bot: sdk.BotDefinition) {
    super({
      exportName: 'Interfaces',
    })

    for (const [alias, interfacePackage] of Object.entries(bot.interfaces ?? {})) {
      const interfaceModule = new InterfaceTypingsModule(interfacePackage.definition)
...
type BotTypingsIndexDependencies = {
  ...
  interfacesModule: BotInterfacesModule
...
export class BotTypingsModule extends Module {
  ...
  public constructor(bot: sdk.BotDefinition) {
    ...
    const interfacesModule = new BotInterfacesModule(bot)
